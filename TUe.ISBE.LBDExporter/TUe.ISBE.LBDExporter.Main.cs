    ...
    public class Export_LBD_File_Main : IExternalCommand
    {
                ...
                foreach (Element e in spaces)
                {
                    ...
                    if (e.Category.Name == "Spaces")
                    {
                        ...
                        tString += ";" +
                            ...
                            NLT + $"rvt:number \"{space.Number}\"^^xsd:string .";*/
                                ...
                                foreach (Autodesk.Revit.DB.BoundarySegment boundarySegment in segmentList)
                                {
                                    ...
                                    if (boundingEl.Category.Name == "<Room Separation>") // == BuiltInCategory.OST_RoomSeparationLines          
                                    {
                                        ...
                                        tString +=
                                            ...
                                            NLT + "geom:asWKT \"LINESTRING (" + (boundarySegment.GetCurve().GetEndPoint(0).X).ToString().Replace(',', '.') + " "
                                                        + (boundarySegment.GetCurve().GetEndPoint(0).Y).ToString().Replace(',', '.') + ", "
                                                        + (boundarySegment.GetCurve().GetEndPoint(1).X).ToString().Replace(',', '.') + " "
                                                        + (boundarySegment.GetCurve().GetEndPoint(1).Y).ToString().Replace(',', '.') + ")\" ." + NL;
                    ...
                    if (e.Category.Name == "Rooms")
                    {
                        ...
                        tString += ";" +
                            ...
                            NLT + $"rvt:number \"{room.Number}\"^^xsd:string ." + NL;*/
          ...
          ElementSet elements)
        {
            ...
            if (savefile.ShowDialog() == DialogResult.OK)
            {
                ...
                /*#region room separation lines
                List<Element> walls = new FilteredElementCollector(doc)
                    .OfClass(typeof(RoomSe)).WhereElementIsNotElementType().ToElements().ToList();
